...
class ViewBasic: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        redView!.layer.anchorPoint = CGPoint(x: 0.5, y: 0.5) // quyet dinh trong tam cua redview
        ...
        // Them mot Slider nua
        let slider2 = UISlider(frame: CGRect(x: 10, y: 150, width: 300, height: 40))
        slider2.addTarget(self, action: "onSliderChange2:", forControlEvents: UIControlEvents.ValueChanged)
        self.view.addSubview(slider2)    }
    
    
    func onSliderChange2(slider2: UISlider) {
        redView!.transform = CGAffineTransformMakeRotation(CGFloat(2.0 * M_PI * Double(slider2.value * 10.0)))
        println("cx = \(redView!.center.x), cy = \(redView!.center.y)")
    }
